...
export const fromDb = {
  ...
  scope: r => ({ id: r.id, projectId: r.project_id, name: r.name || r.type, type: r.type, dataSource: r.data_source, scopeValue: r.qty, coatingsPotential: r.coatings_potential || 0, qty: r.qty, qtyUnit: r.qty_unit, notes: r.notes, products: r.products || [], stage: r.stage || 'Project Identified', specStatus: r.spec_status || '' }),
...
export const toDb = {
  ...
  scope: s => ({ id: s.id, project_id: s.projectId, name: s.name, type: s.type || s.name, data_source: s.dataSource, coatings_potential: s.coatingsPotential || 0, qty: s.qty || null, qty_unit: s.qtyUnit, notes: s.notes, products: s.products || [], stage: s.stage || 'Project Identified', spec_status: s.specStatus || null, updated_at: new Date().toISOString() }),
